# SEID 2364 Syllabus Comparison and After-Draft

Date: 2026-06-28
Branch: template-redesign-2026

## Purpose

This document compares the two syllabus files currently in the repository and then sketches a provisional "After" syllabus based on the broader repo evidence.

- [SYLLABUS.md](./SYLLABUS.md) is the compact official syllabus snapshot.
- [SEID2364 Syllabus 2026S Modul.md](./docs/syllabus-sources/SEID2364%20Syllabus%202026S%20Modul.md) is the fuller semester-facing syllabus record.
- [SYLLABUS_COVERAGE_COMPARISON_MATRIX.md](./SYLLABUS_COVERAGE_COMPARISON_MATRIX.md) is the week-by-week comparison matrix for topics, terminology, assignments, and readings.

## High-Level Comparison

### What is shared

- Same course identity, credits, instructor, and Spring 2026 framing.
- Same core ethical model: Balanced Blended Space, mediated responsibility, hybrid human-machine systems.
- Same broad learning outcomes and assessment proportions.
- Same 14-unit instructional arc.

### What differs

- The official syllabus is shorter, cleaner, and more reusable.
- The detailed syllabus is operational, with week-by-week readings, more policy prose, and more semester-specific wording.
- The detailed syllabus includes additional administrative and pedagogical detail that is useful for reconstructing what the course actually did.
- The official syllabus is better as a baseline; the detailed file is better as a delivery record.

## Section-by-Section Delta

| Section | [SYLLABUS.md](./SYLLABUS.md) | [SEID2364 Syllabus 2026S Modul.md](./docs/syllabus-sources/SEID2364%20Syllabus%202026S%20Modul.md) | Implication for After-Draft |
| --- | --- | --- | --- |
| Title / metadata | Compact course header | Full institutional table with course ID, semester, contacts, and program fields | Keep only reusable header data in the template version |
| Course description | Short conceptual summary | Longer prose with administrative framing and modality references | Merge into one reusable course description |
| Learning outcomes | Six concise outcomes | Same outcomes in fuller wording | Preserve the outcome set, but normalize wording |
| Unit structure | 14 topics with brief reading cues | 14 topics plus detailed reading assignments and sequencing | Use the detailed sequence as the evidence base, then trim it to a reusable form |
| Teaching approach | Short experiential summary | Much longer explanation of modes, interaction, and mediation | Retain the core method, reduce semester-specific phrasing |
| Assessment | 50/30/20 summary | Same weighting plus more implementation detail | Preserve the weights, keep the implementation notes concise |
| AI use | Short policy framing | Expanded AI use requirements and role language | Keep the ethical requirements, but simplify administrative prose |
| Resources | High-level categories | Detailed reading architecture and links | Move specific cohort readings into a separate resource appendix |
| Policies | Brief or absent | Attendance, late work, integrity, accessibility | Keep only policy language that should survive template reuse |

## Evidence From the Repo

The repo shows that the semester evolved beyond a static syllabus page. The lecture-note PDFs, assignment files, case-study materials, and supporting resources provide the evidence needed to reconstruct what was actually delivered.

### Primary evidence categories

- `SYLLABUS.md` and `docs/syllabus-sources/SEID2364 Syllabus 2026S Modul.md` establish the planned and institutional course structure.
- `resources/lecture notes/` contains the PDF lecture decks that reflect actual class delivery.
- `assignments/` contains the prompts and deliverables students actually received.
- `resources/` and `case-studies/` contain the supporting readings, worksheets, and case materials used in the course.
- `resources/week-06/` and the copied lecture-note PDF strengthen the record for the course delivery materials.

### Strong evidence of course delivery artifacts

- [assignments/SEID2364_Assignment01_moral_machine_ethics_journal.md](./assignments/SEID2364_Assignment01_moral_machine_ethics_journal.md)
- [assignments/SEID2364_Assignment04_Ethical_Frameworks.md](./assignments/SEID2364_Assignment04_Ethical_Frameworks.md)
- [assignments/SEID2364_Assignment05_system_structuring.md](./assignments/SEID2364_Assignment05_system_structuring.md)
- [assignments/SEID2364_Assignment06_mediation_pathways.md](./assignments/SEID2364_Assignment06_mediation_pathways.md)
- [assignments/SEID2364_Assignment07_eu_ai_act_system_evaluation.md](./assignments/SEID2364_Assignment07_eu_ai_act_system_evaluation.md)
- [assignments/SEID2364_Assignment08_ai_mediation_risk_fluidity_and_regulatory_limits.md](./assignments/SEID2364_Assignment08_ai_mediation_risk_fluidity_and_regulatory_limits.md)
- [assignments/SEID2364_Assignment09_mediation_pathways_capability_and_responsibility.md](./assignments/SEID2364_Assignment09_mediation_pathways_capability_and_responsibility.md)
- [assignments/SEID2364_Assignment10_system_synthesis_and_communication.md](./assignments/SEID2364_Assignment10_system_synthesis_and_communication.md)
- [resources/week-10/README.md](./resources/week-10/README.md)
- [resources/week-11/SEID2364_misinformation_bad_actors_mediated_systems.md](./resources/week-11/SEID2364_misinformation_bad_actors_mediated_systems.md)
- [resources/week-12/12-SEID2364_Case_Study_Game_Theory_Worksheet.md](./resources/week-12/12-SEID2364_Case_Study_Game_Theory_Worksheet.md)

### Evidence of later-semester or reset-era restructuring

- [assignments/SEID2364_Assignment11_acm_ieee_covid_case_analysis.md](./assignments/SEID2364_Assignment11_acm_ieee_covid_case_analysis.md)
- [assignments/upcoming/SEID2364_Assignment11_emerging_technologies_and_uncertainty.md](./assignments/upcoming/SEID2364_Assignment11_emerging_technologies_and_uncertainty.md)
- [assignments/SEID2364_Assignment12_case_study_specification.md](./assignments/SEID2364_Assignment12_case_study_specification.md)
- [assignments/SEID2364_Assignment13_final_assignment_ethical_position_statements.md](./assignments/SEID2364_Assignment13_final_assignment_ethical_position_statements.md)
- [assignments/SEID2364_Assignment14_final_class_framework_presentation_preparation.md](./assignments/SEID2364_Assignment14_final_class_framework_presentation_preparation.md)

### Evidence from lecture notes and slide decks

- [resources/lecture notes/SEID2364_week02-PowerPoint Final 2026S-with notes.pdf](./resources/lecture%20notes/SEID2364_week02-PowerPoint%20Final%202026S-with%20notes.pdf)
- [resources/lecture notes/SEID2364_Week04_Slides_Frameworks-with notes.pdf](./resources/lecture%20notes/SEID2364_Week04_Slides_Frameworks-with%20notes.pdf)
- [resources/lecture notes/SEID2364_Week05_Slides_BBS-v2-with notes.pdf](./resources/lecture%20notes/SEID2364_Week05_Slides_BBS-v2-with%20notes.pdf)
- [resources/SEID2364_Week_06 - BBS Slide presentation_outline.md](./resources/SEID2364_Week_06%20-%20BBS%20Slide%20presentation_outline.md)
- [resources/lecture notes/SEID2364_Week06 - BBS Slide presentation-with notes.pdf](./resources/lecture%20notes/SEID2364_Week06%20-%20BBS%20Slide%20presentation-with%20notes.pdf)
- [resources/lecture notes/Seid2364_Week07 - EU Act - with notes.pdf](./resources/lecture%20notes/Seid2364_Week07%20-%20EU%20Act%20-%20with%20notes.pdf)
- [resources/lecture notes/SEID2364_Week08-Bias, Mediation, and System Pathways-with notes.pdf](./resources/lecture%20notes/SEID2364_Week08-Bias,%20Mediation,%20and%20System%20Pathways-with%20notes.pdf)
- [resources/lecture notes/SEID2364_Week09 -Distance Authority and Risk-with notes.pdf](./resources/lecture%20notes/SEID2364_Week09%20-Distance%20Authority%20and%20Risk-with%20notes.pdf)
- [resources/lecture notes/SEID2364_Week11-Presentation misinformation bad actors-with notes.pdf](./resources/lecture%20notes/SEID2364_Week11-Presentation%20misinformation%20bad%20actors-with%20notes.pdf)
- [resources/lecture notes/SEID2364_Week12 Slide Deck - Ethical Systems As Strategic Games-with notes.pdf](./resources/lecture%20notes/SEID2364_Week12%20Slide%20Deck%20-%20Ethical%20Systems%20As%20Strategic%20Games-with%20notes.pdf)
- [resources/lecture notes/SEID2364_Week13 Powerpoint Synthesis and Putting it all together-with notes.pdf](./resources/lecture%20notes/SEID2364_Week13%20Powerpoint%20Synthesis%20and%20Putting%20it%20all%20together-with%20notes.pdf)

## Provisional After-Draft: What the Reconciled Syllabus Should Do

This is not the final prose syllabus yet. It is the structural draft for the after-version.

### 1. Keep the reusable course shell

- Retain the course title, credits, and core ethical framing.
- Retain the six learning outcomes, but present them in a slightly more compact, template-neutral voice.
- Retain the 14-unit course arc as the organizing frame.

### 2. Replace semester snapshot language with reusable language

- Remove direct Spring 2026-only phrasing from the main course description.
- Move live contacts, semester dates, and institutional logistics into a separate semester handout if needed.
- Keep the AI policy, but strip out wording that depends on a specific cohort or semester reset.

### 3. Rebuild the weekly sequence around repo evidence

| Unit | After-Draft emphasis | Evidence in repo |
| --- | --- | --- |
| 1 | Orientation, Moral Machine, baseline ethical reflection | Assignment 01 and foundational resources |
| 2 | Intelligence, agency, and responsibility | Foundational reading set and BBS framing |
| 3 | Ethical frameworks and comparative analysis | Assignment 04 |
| 4 | Describing ethical environments, data, and bias | Assignment 05 |
| 5 | Mediation within socio-technical systems | Assignment 06 |
| 6 | Institutions, power, and accountability | Assignment 07 |
| 7 | Human-machine collaboration and authorship | Assignment 08 |
| 8 | Surveillance, privacy, and trust | Assignment 08 and related policy/case materials |
| 9 | Communication and ethical risk | Assignment 09 |
| 10 | Applied synthesis and systems communication | Assignment 10 and Week 10 signal-tracking materials |
| 11 | Emerging technologies and uncertainty | Assignment 11 reset-era materials and week 11 resources |
| 12 | Personal ethical framework development | Assignment 12 materials |
| 13 | Novel-context stress testing | Assignment 13 materials |
| 14 | Final synthesis and presentation | Assignment 14 materials |

### 4. Make the assessment section cleaner

- Keep the 50/30/20 weighting.
- Preserve the distinction between applied projects, personal ethical statement, and participation/engagement.
- Move low-stakes implementation notes into a short appendix or semester guide.

### 5. Trim readings into a reusable structure

- Keep the core reading categories.
- Move the detailed week-by-week reading list into a companion resource document.
- Treat the syllabus as the stable structure, not the full reading archive.

## What the After-Syllabus Should Look Like

The final after-syllabus should read like a cleaned and reconciled course document:

- baseline syllabus structure preserved
- semester-specific details removed or minimized
- actual implemented topics reflected in the weekly arc
- late-semester restructure acknowledged, not hidden
- auxiliary materials referenced outside the core syllabus body

## Open Questions

- Should the after-syllabus preserve the Spring 2026 course snapshot as a historical version, or should it be rewritten entirely as a reusable template version?
- Should the late-semester reset be represented as a formal mid-course revision note?
- Which reading list should be considered canonical for the template: the official syllabus reading list, or the more detailed semester delivery record?
